...
''';
  ...
  Future<void> _toggleLanServer() async {
    ...
    try {
      ...
      await _showLanServerDialog(url);
    } catch (e) {
      if (!mounted) {
        return;
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('فشل تشغيل LAN Server: $e'),
        ),
      );
    }
  }

  Future<void> _showLanServerDialog(String url) async {
      ...
      builder: (dialogContext) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              const Text(
                'افتح هذا الرابط من أي جهاز على نفس شبكة Wi-Fi:',
              ...
              const Text(
                'السيرفر متاح داخل الشبكة المحلية فقط.',
                textAlign: TextAlign.center,
              ),
            ],
          ),
  ...
  Future<void> _showLanServerStatus() async {
    final url = _lanServerUrl;

    if (!_lanServerRunning || url == null) {
      return;
    }
